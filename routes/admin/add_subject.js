...
router.get('/', function (req,res,next) {
    if(req.session.level){
        res.render('admin/add_subject',{ad:req.session.user_ad});
    }else{
        res.redirect('/admin');
    }
});
    ...
    if(req.session.level){
        if(req.body.ma_mon!=''&&req.body.ten_mon!=''&&req.body.khoa&&req.body.tin_chi){
            ...
            check_subject.query('select * from monhoc where ma_mon="'+req.body.ma_mon+'"',function(err,rows,fields){
                if(err) res.render('admin/add_subject',{rp:'Lỗi truy vấn.'});
                if(rows.length>0){
                    res.render('admin/add_subject',{rp:'Mã môn đã tồn tại.'});
                }
                if(rows.length==0) {
                    ...
                    create_subject.query('INSERT INTO monhoc SET ?',subject,function(err,result){
                       if(err) res.render('admin/add_subject',{ad:req.session.user_ad,rp:'Đã xảy ra lỗi.'});
                       else{
                           res.render('admin/add_subject',{ad:req.session.user_ad,rp:'Môn học thêm thành công.'});
                       }
                    });
                }
            });
        }else{
            res.render('admin/add_subject',{ad:req.session.user_ad,rp:'Bạn đã nhập thiếu thông tin.'})
        }
    }else{
        res.redirect('/admin');
    }
